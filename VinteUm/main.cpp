...
void draw_spaceship(int x, int y, int r, int g, int b) {
	...
	al_draw_filled_rectangle(20 + x, 50 + y, 80 + x, 60 + y, al_map_rgb(r - 35, g, b - 35));
}
...
int main() {
	...
	al_set_window_title(display, "*** Vinte e Um *** Welcome ***");
	al_clear_to_color(color_bg);

	int x = 0; int y = 0; int r = 0; int g = 0; int b = 0; int z = 0; int w = 0;

	// Move spaceships by changing draw's x positions
	while(x < display_width) {
		...
		draw_spaceship(x, y, r + 255, g, b);
		draw_spaceship(x + z, y + 230, r, g + 255, b);
		draw_spaceship(x + w, y + 460, r, g, b + 255);
		al_flip_display();
		al_clear_to_color(color_bg);
		x += 1;
	}
